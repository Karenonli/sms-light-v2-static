...
async function handleUpdate(update) {
  ...
  if (text === '/start') {
    // Вступительная картинка + приветствие и кнопки (sendPhoto по URL требует PNG)
    const caption = 'Добро пожаловать в SMS Light! 🛍️\n\n'
      + 'Виртуальные номера для регистрации в сервисах — прямо здесь, в Telegram.\n\n'
      + '⬇️ Нажмите кнопку, чтобы открыть мини-маркет. Если вы на ПК — откройте магазин в браузере:';
    const markup = {
      inline_keyboard: [
        [{ text: '🛍️ Открыть мини-маркет', web_app: { url: SITE_BASE + '/shop' } }],
        [{ text: '🖥️ Открыть в браузере (ПК)', url: SITE_BASE }],
        [{ text: '🧑‍💻 Поддержка', url: 'https://t.me/' + SUPPORT_USERNAME }],
      ],
    };
    const photo = await api('sendPhoto', {
      chat_id: chatId,
      // ?v=2 — кэш-бастер: Telegram Bot API кэширует фото по URL, без смены URL
      // отдал бы старую картинку даже после замены файла на сервере.
      photo: SITE_BASE + '/img/sms-light-start.png?v=2',
      caption,
      reply_markup: markup,
    });
    // Фолбэк: если фото не ушло (нет доступа к URL), отправляем просто текст
    if (!photo || !photo.ok) {
      await api('sendMessage', { chat_id: chatId, text: caption, reply_markup: markup });
    }
