...
async function initDb() {
  ...
  initPromise = (async () => {
    ...
    // Telegram-регистрация: tg_id хранит неизменяемый Telegram ID пользователя,
    // поэтому аккаунт не теряется при смене @username. NULL у обычных пользователей,
    // уникальный — у Telegram-пользователей (в Postgres NULLы в уникальном индексе не мешают).
    await db.exec('ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_id BIGINT');
    await db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_tg_id ON users (tg_id)');
